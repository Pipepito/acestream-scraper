...
|---|-------|--------|----------|
| 1 | Core v2 endpoints use explicit request/response schemas with no ambiguous runtime payload parsing. | ✓ VERIFIED | `backend/app/api/endpoints/channels.py` and `config.py` use explicit `response_model` and typed `Body(...)` payloads. |
| 2 | Channels/TV/config/EPG contracts are consistent across backend and frontend service consumers. | ✓ VERIFIED | `frontend/src/services/channelService.ts` uses typed `{items,total}` envelope; contract tests in `tests/contracts/` assert payload shapes. |
...
|----------|----------|--------|---------|
| `backend/app/schemas/channel.py` | Typed channel/TV DTO contracts | ✓ EXISTS + SUBSTANTIVE | Contains bulk/association/batch response and request models. |
| `backend/app/schemas/config.py` | Typed config request/response envelopes | ✓ EXISTS + SUBSTANTIVE | Contains typed update and dashboard config schemas. |
| `backend/app/api/endpoints/tv_channels.py` | Typed TV-channel endpoint boundary | ✓ EXISTS + SUBSTANTIVE | Uses typed request bodies and `response_model` annotations. |
| `backend/tests/contracts/test_channel_contracts.py` | Channel/TV contract regression checks | ✓ EXISTS + SUBSTANTIVE | Validates schema parsing, endpoint envelope shape, and validation behavior. |
| `backend/tests/contracts/test_config_contracts.py` | Config contract regression checks | ✓ EXISTS + SUBSTANTIVE | Asserts typed config response/update contracts. |
| `backend/app/api/dependencies.py` | Shared endpoint DI providers | ✓ EXISTS + SUBSTANTIVE | Defines `get_url_service` and `get_stats_service`. |
| `backend/app/repositories/url_repository.py` | URL data access owner | ✓ EXISTS + SUBSTANTIVE | Provides URL list/refresh/update data operations. |
| `backend/app/repositories/stats_repository.py` | Stats aggregate data access owner | ✓ EXISTS + SUBSTANTIVE | Encapsulates stats/health aggregate query logic. |
| `backend/tests/architecture/test_layer_boundaries.py` | Layer-boundary architecture guards | ✓ EXISTS + SUBSTANTIVE | Prevents direct ORM query patterns in targeted endpoint/service paths. |
| `backend/app/schemas/errors.py` | Canonical API error envelope | ✓ EXISTS + SUBSTANTIVE | Defines structured error payload and metadata contract. |
| `backend/app/api/error_handlers.py` | Global exception translation | ✓ EXISTS + SUBSTANTIVE | Registers API/global handlers and correlation-aware response shaping. |
| `backend/tests/test_error_contracts.py` | Failure contract regression checks | ✓ EXISTS + SUBSTANTIVE | Verifies status mapping + error payload fields in failure paths. |
...
|------|------|---------|----------|--------|
| `backend/app/repositories/stats_repository.py` | 26 | `return {}` fallback for empty aggregate set | ⚠️ Warning | Non-blocking; deterministic default used for no-data path |
...
- `backend/venv/bin/pytest -q backend/tests/contracts/test_channel_contracts.py backend/tests/contracts/test_config_contracts.py backend/tests/test_channels.py backend/tests/test_tv_channels.py backend/tests/test_config.py backend/tests/test_epg.py backend/tests/test_urls.py backend/tests/test_scrapers.py backend/tests/test_health.py backend/tests/architecture/test_layer_boundaries.py backend/tests/test_error_contracts.py backend/tests/test_background_tasks.py`
