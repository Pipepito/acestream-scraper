...
- Fork PRs use `jenkins/pr.Jenkinsfile` from trusted `develop` and execute contributor-controlled files inside the prebuilt `docker/ci/pr-runner.Dockerfile` image with no network, capabilities, Docker socket, or Jenkins credentials. `scripts/ci/run_pr_validation.sh` is the credential-free quick gate.
...
- `services/` holds business logic — one module per domain (channels, tv channels, EPG sources/channels/programs, scrapers, search, playlists, stats, activity log, dashboard config, WARP, streams, task scheduler, player, etc.). Endpoints inject `Session` via `Depends(get_db)` and instantiate the relevant service.
