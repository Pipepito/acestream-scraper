...
def run_v1_epg_programs_migration():
    ...
    def report(snapshot):
        ...
        startup_service.progress(snapshot)
    ...
    try:
        ...
        snapshot = migrator._progress_snapshot(state)
        snapshot['status'] = 'error'
        startup_service.progress(snapshot)
        startup_service.record('Programme import stopped. Saved progress is kept for a retry.', 'error')
        raise
    startup_service.progress(summary)
    startup_service.record("Programme import " + summary["status"], "warning" if summary["status"] == "error" else "info")
