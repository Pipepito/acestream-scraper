...
def upgrade():
    ...
    # 3. Create epg_programs table if it doesn't exist
    if not has_table('epg_programs'):
        op.create_table(
            'epg_programs',
            sa.Column('id', sa.Integer(), primary_key=True),
            sa.Column('epg_channel_id', sa.Integer(), nullable=False),
            sa.Column('program_xml_id', sa.String(255), nullable=True),
...
def downgrade():
    ...
    # 2. Drop epg_programs table if it exists
    if has_table('epg_programs'):
        op.drop_table('epg_programs')

    # 3. Drop epg_channels table if it exists
