...
def upgrade():
    ...
    if not has_column('scraped_urls', 'url_type'):
        ...
        scraped_urls = connection.execute(sa.text('SELECT url FROM scraped_urls')).fetchall()

        for row in scraped_urls:
            url = row[0]
            url_type = 'zeronet' if (
                url.startswith('zero://')
                or url.startswith('http://127.0.0.1:43110/')
                or ':43110/' in url
            ) else 'regular'
            connection.execute(
                sa.text("UPDATE scraped_urls SET url_type = :url_type WHERE url = :url"),
                {"url_type": url_type, "url": url},
            )
