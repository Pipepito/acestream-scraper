...
class TestConfigEndpoints:
    ...
    def test_get_base_url_default(self, alembic_client):
        """Test getting base URL setting (default value)."""
        response = alembic_client.get("/api/v1/config/base_url")
    ...
    def test_update_base_url(self, alembic_client):
        """Test updating base URL setting."""
        new_base_url = "http://new-server.example.com:8000"
        update_data = {"base_url": new_base_url}

        response = alembic_client.put("/api/v1/config/base_url", json=update_data)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_update_base_url_invalid_format(self, alembic_client):
    ...
    def test_get_ace_engine_url_default(self, alembic_client):
        """Test getting acestream engine URL setting."""
        response = alembic_client.get("/api/v1/config/ace_engine_url")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "key" in data
        assert "value" in data
    ...
    def test_update_ace_engine_url(self, alembic_client):
        ...
        response = alembic_client.put("/api/v1/config/ace_engine_url", json=update_data)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_get_rescrape_interval_default(self, alembic_client):
        """Test getting rescrape interval setting."""
        response = alembic_client.get("/api/v1/config/rescrape_interval")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "key" in data
        assert "value" in data
    ...
    def test_update_rescrape_interval(self, alembic_client):
        """Test updating rescrape interval setting."""
        new_interval = "7200"  # 2 hours
        update_data = {"value": new_interval}

        response = alembic_client.put("/api/v1/config/rescrape_interval", json=update_data)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["message"] == "Setting updated successfully"
    ...
    def test_update_rescrape_interval_invalid_value(self, alembic_client):
    ...
    def test_update_rescrape_interval_negative_value(self, alembic_client):
    ...
    def test_get_addpid_setting_default(self, alembic_client):
        """Test getting addpid setting."""
        response = alembic_client.get("/api/v1/config/addpid")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert "key" in data
        assert "value" in data
    ...
    def test_update_addpid_setting(self, alembic_client):
        """Test updating addpid setting."""
        new_addpid = "true"
        update_data = {"value": new_addpid}

        response = alembic_client.put("/api/v1/config/addpid", json=update_data)
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
        assert data["message"] == "Setting updated successfully"
    ...
    def test_update_addpid_setting_boolean_values(self, alembic_client):
...
class TestConfigAllSettingsEndpoint:
    ...
    def test_get_all_settings(self, alembic_client):
        """Test getting all configuration settings."""
        response = alembic_client.get("/api/v1/config/all")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_get_all_settings_structure(self, alembic_client):
    ...
    def test_get_all_settings_after_updates(self, alembic_client):
        ...
        for key, value in updates:
            update_data = {"value": value}
            response = alembic_client.put(f"/api/v1/config/{key}", json=update_data)
            assert response.status_code == status.HTTP_200_OK
...
class TestConfigAcestreamStatusEndpoint:
    ...
    def test_get_acestream_status(self, alembic_client):
        """Test getting acestream engine status."""
        response = alembic_client.get("/api/v1/config/acestream_status")
        assert response.status_code == status.HTTP_200_OK
        data = response.json()
    ...
    def test_acestream_status_with_custom_engine_url(self, alembic_client):
    ...
    def test_acestream_status_error_handling(self, alembic_client):
...
class TestConfigIntegration:
    ...
    def test_config_persistence(self, alembic_client, alembic_db_session):
    ...
    def test_config_validation_integration(self, alembic_client):
    ...
    def test_config_defaults_initialization(self, alembic_client):
        ...
        for endpoint in config_endpoints:
            response = alembic_client.get(f"/api/v1/config/{endpoint}")
            assert response.status_code == status.HTTP_200_OK
            data = response.json()
