def test_get_dashboard_config(alembic_client):
    response = alembic_client.get("/api/v1/config/dashboard")
    assert response.status_code == 200
    data = response.json()
    assert "retention_days" in data
    assert "auto_refresh_interval" in data

def test_update_dashboard_config(alembic_client):
    response = alembic_client.put(
