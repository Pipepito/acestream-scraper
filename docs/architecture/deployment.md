...
ZeroNet has two modes. The `linux/amd64` and `linux/arm64` images bundle a zeronet-conservancy node under `/opt/zeronet`, pinned by commit to a state that carries DHT peer discovery (on its own Python 3.11), enabled with `ENABLE_ZERONET=true` (+ optional `ENABLE_TOR=true`); state lives in `/data/zeronet` (`ZERONET_DATA_DIR`), UI on `43110`, fileserver on `26552`. Otherwise ZeroNet remains an external sidecar/service: the `ZERONET_URL` contract is unchanged, and it is the only mode on 32-bit ARM images, which ship without the bundled node because gevent publishes no armv7l wheels.

The checked-in compose file points `ZERONET_URL` at `http://host.docker.internal:43110` by default so the app can run without the optional `zeronet` profile. The example `zeronet` service is pinned for amd64. 32-bit ARM deployments should use an external ZeroNet endpoint or replace that sidecar while keeping `ZERONET_URL` unchanged. With `ENABLE_ZERONET=true` and no explicit `ZERONET_URL`, the entrypoint targets the embedded node's UI port automatically. A fresh embedded node is seeded with public UDP bootstrap trackers; `ZERONET_TRACKERS` accepts a space-separated replacement list.
