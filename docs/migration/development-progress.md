...
- PR validation: multibranch job `acestream-scraper-pr` loads `jenkins/pr.Jenkinsfile`, reports the required `PR Validation` context, and discovers origin and fork PR merge revisions. Fork Jenkinsfiles are trusted from the target branch; contributor files execute in a network-disabled, capability-dropped container with no credentials or Docker socket. Privileged Docker/runtime checks moved to the trusted develop job.
