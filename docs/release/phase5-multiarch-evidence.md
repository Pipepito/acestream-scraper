...
| `linux/arm64` engine smoke | **Pass** — `scraper-acestream` built natively; `/webui/api/service?method=get_version` answered in ~4 s with `{"platform":"android","version":"3.1.80"}`; HTTP API up within ~5 s of start, RSS ~95 MB idle; the backend's health probes (`/server/api?api_version=3&method=get_status`, `get_network_connection_status`) return 200 |
