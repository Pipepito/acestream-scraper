...
- AceStream engine 3.2.17 on `linux/arm64` (stable) and `linux/arm/v7` (experimental), using each platform's variant from digest-pinned `jopsis/acestream:v3.2.17-fix`. ARM cold builds require Docker Hub. Runtime and dashboard attribution evidence: `docs/release/phase5-multiarch-evidence.md`. Operator guide: `docs/ops/acestream-arm-engine.md`.
...
- No documented manifest-update procedure for `docker/manifests/acestream.json` when AceStream gains ARM platforms. *(Closed: ARM is enabled through digest-pinned OCI entries, the pin/update procedure lives in `docs/ops/multiarch-manifest-updates.md`, and `python3 scripts/ci/validate_docker_manifest_metadata.py` checks the manifest and source metadata.)*
