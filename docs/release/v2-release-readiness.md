...
- AceStream engine on `linux/arm64` (stable) via digest-pinned `jopsis/acestream:v3.2.17-fix` and on `linux/arm/v7` (experimental) via the official Android APK. Conventional archives are vendored/mirrored; the ARM64 cold build requires Docker Hub. Runtime and dashboard attribution evidence: `docs/release/phase5-multiarch-evidence.md`. Operator guide: `docs/ops/acestream-arm-engine.md`.
