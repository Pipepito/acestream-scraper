...
- Known ARM limitations: engine version skew (3.1.80 on ARM vs 3.2.11 on amd64; `platform` reports `android`); no WebRTC transport (pywebrtc needs Android GPU/audio libs; non-fatal engine error); a few CPython accelerator modules fall back to pure Python; no WARP on ARM images (cloudflare-warp is amd64-only, pre-existing — the "every runtime flavor installs WARP support" rule above already did not hold on ARM); performance/streaming stability not validated on real hardware; repackaging the APK is a licensing grey area shared by every community ARM image. See also `docs/release/arm-acestream-issue-draft.md`.
