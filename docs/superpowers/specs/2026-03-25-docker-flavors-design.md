...
### Addendum (updated 2026-09-03): Android OCI image install kind for ARM
...
- `linux/arm64` and `linux/arm/v7` use their matching platform variants from digest-pinned `jopsis/acestream:v3.2.17-fix`. Each image supplies the Android engine payload (python-for-Android CPython plus compiled modules) and matching bionic userland under `/system`; this project grafts both into its runtime and replaces the bootstrap with its persistent Linux bridge. No chroot, `--privileged`, seccomp changes, or extra capabilities are needed (verified on arm64).
- `linux/amd64` is unchanged: install kind `executable`, 3.2.11 tarball.
- Schema as implemented (validated by `scripts/ci/validate_docker_manifest_metadata.py`): top-level `version` (amd64 engine), `android_version`, `vendor_dir`, `mirror_base_url`, and `platforms{<platform>: {engine_version, support (stable|experimental), install{kind, engine_http_port, ...}, ...}}`. `executable` installs add the archive URL/checksum/vendor fields, `strip_components`, and `binary_path`; current ARM `oci-image` installs add `distribution`, `distribution_url`, `source_url`, `image_ref`, and `image_digest`. The legacy `android-apk` schema remains supported by the installer.
- Support levels: `linux/arm64` is `stable` (engine verified; `/webui/api/service?method=get_version` returns `{"platform":"android","version":"3.2.17"}`). `linux/arm/v7` is `experimental`: its jopsis 3.2.17 image builds and installs but cannot be executed under qemu-user (32-bit bionic calls `personality(PER_LINUX32)`), so it needs real ARMv7/AArch32-capable hardware and has not been runtime-tested. The engine stays opt-in (`ENABLE_ACESTREAM_ENGINE=false` by default), so the app itself is unaffected.
...
- Install source: the amd64 engine archive is vendored in `docker/vendor/acestream/`; legacy ARM APKs and bionic `.deb`s remain vendored for reproducibility. Current ARM builds copy their matching variant from the digest-pinned jopsis OCI image, so a cold builder requires Docker Hub access. Manual-by-manifest (Update Strategy below) still holds; bumping a pin is documented in `docs/ops/multiarch-manifest-updates.md`.
...
- Known ARM limitations: engine version skew (3.2.17 on ARM vs 3.2.11 on amd64; `platform` reports `android`); no WebRTC transport (pywebrtc needs Android GPU/audio libs; non-fatal engine error); a few CPython accelerator modules fall back to pure Python; no WARP on ARMv7 (Cloudflare publishes amd64 and arm64 packages only); performance/streaming stability not validated on real hardware; repackaging the Android payload is a licensing grey area shared by every community ARM image. See also `docs/release/arm-acestream-issue-draft.md`.
