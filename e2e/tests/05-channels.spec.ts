...
test.describe('acestream channels', () => {
  test('the grid lists channels and the name filter narrows it', async ({ page, api }) => {
    ...
    await expect(channels.grid().locator('[role="row"][data-id]')).toHaveCount(filtered.items.length);
    await channels.resetFilters();
  });

  test('a channel can be edited from the quick edit dialog', async ({ page, api }) => {
  ...
  test('a channel can be created by hand and deleted again', async ({ page, api }) => {
    ...
    await dialog.getByRole('textbox', { name: 'Acestream ID (hash)' }).fill(E2E_ID);
    await dialog.getByRole('button', { name: 'Save' }).click();
    await expect(dialog).toBeHidden();
    ...
    await channels.toggleActive('E2E Manual Channel');
