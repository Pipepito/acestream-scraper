pipeline {
  ...
  parameters {
    ...
    booleanParam(name: 'FORCE_VERSION_OVERWRITE', defaultValue: false, description: 'Allow replacing existing versioned Docker images. Leave OFF for normal releases; use only for deliberate recovery of an existing version.')
  }
...
'''
        script {
          withEnv([
            "PUBLISH_LATEST=${params.PUBLISH_LATEST ? '1' : '0'}",
            "FORCE_VERSION_OVERWRITE=${params.FORCE_VERSION_OVERWRITE ? '1' : '0'}"
          ]) {
            if (params.DRY_RUN) {
