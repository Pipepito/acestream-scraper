...
# BuildKit limits for the docker-container builder. The runner has a 32 GB
# disk shared with other jobs; without GC the builder's cache volume grew to
# 6 GB after one three-platform build and filled the disk (PR-162 #2/#3).
# gckeepstorage is in MB; max-parallelism bounds concurrent build steps so a
# multi-platform publish does not run every QEMU stage at once.
builder_config_dir() {
  printf '%s\n' "${BOOTSTRAP_BUILDER_CONFIG_DIR:-${BOOTSTRAP_TEST_STATE_DIR:-$HOME/.config/acestream-builder}}"
}

write_buildkitd_config() {
  local dir="$1"
  mkdir -p "$dir"
  cat > "$dir/buildkitd.toml" <<EOF
# Generated by scripts/ci/bootstrap_jenkins_runner.sh for builder ${BUILDER}.
[worker.oci]
  gc = true
  gckeepstorage = ${BUILDKIT_GC_KEEP_STORAGE_MB:-4000}
  max-parallelism = ${BUILDKIT_MAX_PARALLELISM:-2}
EOF
}
...
ensure_builder_exists() {
  local dir cfg digest marker
  dir="$(builder_config_dir)"
  write_buildkitd_config "$dir"
  cfg="$dir/buildkitd.toml"
  digest="$(file_digest "$cfg")"
  marker="$dir/${BUILDER}.buildkitd-config-digest"

  if docker buildx inspect "$BUILDER" >/dev/null 2>&1; then
    if [[ -f "$marker" && "$(cat "$marker")" == "$digest" ]]; then
      return
    fi
    log "Recreating buildx builder $BUILDER with the current buildkitd config"
    docker buildx rm "$BUILDER" >/dev/null 2>&1 || true
  fi

  log "Creating buildx builder: $BUILDER (buildkitd config: $cfg)"
  docker buildx create --name "$BUILDER" --driver docker-container --buildkitd-config "$cfg" >/dev/null
  printf '%s\n' "$digest" > "$marker"
}
