...
if [[ ! -x /opt/backend-venv/bin/python || ! -d /opt/frontend-node_modules ]]; then
  echo "Trusted validation runner dependencies are missing." >&2
  exit 1
fi
...
# Use the immutable dependency set built from the checked-out revision. The copied frontend
...
echo "Pinned Python 3.12 application validation passed."
